...
public class RekapReportActivity extends AppCompatActivity {
    ...
    private static final String[] arrayBulan = new String[]{
            "Pilih Bulan",
            "Januari",
            "Februari",
            "Maret",
    ...
    private static final String[] arrayTahun = new String[]{
            "Pilih Tahun",
            "2018",
            "2019",
            "2020",
    ...
    private String buatText(){
        ...
        DatabaseReference refPemasukan = FirebaseDatabase.getInstance().getReference("pemasukan");

        ArrayList<setkas> listPemasukan = new ArrayList<>();
        ArrayList<setkas> listPengeluaran = new ArrayList<>();

        refPemasukan.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot postSnapshot : snapshot.getChildren()) {
                    ...
                    if (sk.getTitle().equals("pemasukan")){
                        if (spnBulan.getSelectedItemPosition()==Integer.parseInt(dateStr[1]) && Integer.parseInt(arrayTahun[spnTahun.getSelectedItemPosition()])==Integer.parseInt(dateStr[2])){
                            listPemasukan.add(sk);
                        }
                    }else if (sk.getTitle().equals("pengeluaran")){
                        if (spnBulan.getSelectedItemPosition()==Integer.parseInt(dateStr[1]) && Integer.parseInt(arrayTahun[spnTahun.getSelectedItemPosition()])==Integer.parseInt(dateStr[2])){
                            listPengeluaran.add(sk);
                        }
                    }
                }
                ...
                for (int a = 0; a<listPemasukan.size();a++){
                    setkas sk = listPemasukan.get(a);
                    pemasukan = pemasukan + Double.parseDouble(sk.getJumlah());
                ...
                sb.append("-------------------------------------\n");
                for (int a = 0; a<listPengeluaran.size();a++){
                    setkas sk = listPengeluaran.get(a);
                    pengeluaran = pengeluaran + Double.parseDouble(sk.getJumlah());
                    sb.append("- ").append(sk.getRanting()).append("\t\t\t\t\t\t\t").append(formatrupiah(Double.parseDouble(sk.getJumlah()))).append("\n");
                }
                sb.append("\n");
                ...
                sb.append("- Pengeluaran").append("\t\t:\t\t\t\t\t\t\t").append(formatrupiah(pengeluaran)).append("\n");
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                ...
                sb.append("Tidak ada pemasukan pemasukan\n");
            }
        });
